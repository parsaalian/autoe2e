								...
								<ul className="mt-4 space-y-4 [&>li]:text-neutral-500">
									{item.children?.map((child) => {
										...
										if (child.page) {
											return (
												<li key={child.id} className="text-sm">
													<LinkWithChannel href={`/pages/${child.page.slug}`} clickId="c9-footer-page">
														{child.page.title}
													</LinkWithChannel>
												</li>
											);
										...
										if (child.url) {
											return (
												<li key={child.id} className="text-sm">
													<LinkWithChannel href={child.url} clickId="c10-footer-child">
														{child.name}
													</LinkWithChannel>
												</li>
											);
